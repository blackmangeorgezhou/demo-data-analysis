...
export default {
  ...
  data () {
    return {
      ...
      treeLevelFields: ['Department', 'Region', 'Grade', 'SubjectName']
    }
  },
  ...
  methods: {
      ...
      for (let item of list) {
        ...
        if (department) {
          ...
          if (region) {
            ...
            let grade = region.children.find(x => x.label === item[fieldArray[2]])
            if (grade) {
              grade.studentCount += item.StudentNum
              let subject = grade.children.find(x => x.label === item[fieldArray[3]])
              if (subject) {
                subject.studentCount += item.StudentNum
              } else {
                grade.children.push(Convertor.generateTreeObj(this.treeLevelFields, 3, item))
              }
            } else {
    ...
    //       let findItem = this.findChildern(children, item[fieldArray[i]])
    //       if (findItem) {
    //         findItem.studentCount += item.StudentNum
    //       } else {
    //         let parentItem = this.findParent(children, item[fieldArray[i]])
    //         parentItem.push(this.generateTreeObj(this.treeLevelFields, i, item))
    //       }
    //     }

    findChildern (children, findValue) {
      let findItem = children.find(x => x.label === findValue)
      if (findItem) {
        return findItem
      } else {
        return this.findChildern(children.children, findValue)
      }
    },

    findParent (children, findValue) {
      let findItem = children.find(x => x.label === findValue)
      if (!findItem) {
        return children
      } else {
