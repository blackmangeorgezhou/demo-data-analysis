...
export default {
  ...
  data () {
    return {
      ...
      treeLevelFields: ['Department', 'Region', 'Campus', 'SubjectNum']
    }
  },
  ...
  methods: {
    async loaddata () {
      ...
      let response = await API.getUnionSignUp(reqParams)
      console.log(response)
      if (response && response.resultCode === ResultCode.OK) {
        this.treeData = this.convertToTreeData('联报人数', this.converUnionSignUpData(response.data), this.treeLevelFields)
      } else {
    ...
    converUnionSignUpData (list) {
      if (!list || !list.length) return []
      let newList = []

      for (let item of list) {
        item.SubjectNum = `${item.SubjectNum}科`
        newList.push(item)
      }
