<template>
  <div class="self-upload-dialog">
    <el-dialog :title="title" :visible.sync="isShowUploadDialog" @close="handleDialogClose">
      <p class="font-size-12 m-bottom8">请上传Excel文件:</p>
      <el-upload class="uploadDemo" drag accept=".xls,.xlsx"
      ...
      <span slot="footer" class="dialog-footer">
          <el-button @click="dialogConfigCancel">取 消</el-button>
          <el-button type="primary" @click="importSubmitFunc" :disabled="!importFiles || !importFiles.length">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
    ...
    validateExcelData (excelJsonData) {
      ...
      for (let i = 0; i < excelJsonData.length; i++) {
        ...
        for (let key of keys) {
          ...
          const validateValue = data[key].toString().trim()
